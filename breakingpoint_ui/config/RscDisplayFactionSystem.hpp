...
class RscDisplayFactionSystem
{
	...
	class controls
	{
		...
		class BP_Scavenger: RscActiveText
		{
			...
			onMouseEnter = "ctrlSetFocus (_this select 0); (_this select 0) ctrlSetText '\breakingpoint_ui\factions\buttons\scavenger_hover.jpg'";
			onMouseExit = "ctrlSetFocus (_this select 0); (_this select 0) ctrlSetText '\breakingpoint_ui\factions\buttons\scavenger.jpg'";
		};
...
class RscDisplayFactionOptOut
{
	...
	class controlsBackground {
		class Mainback : RscPicture {
			idc = 4080;
			text = "\breakingpoint_ui\factions\scavenger.jpg";
			x = 0 * safezoneW + safezoneX;
			y = 0 * safezoneH + safezoneY;
			w = 1 * safezoneW;
			h = 1 * safezoneH;
