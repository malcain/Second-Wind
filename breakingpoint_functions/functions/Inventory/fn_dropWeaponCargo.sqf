...

{
    ...
    private _weaponNonPreset = [_weapon] call BP_fnc_getNonPresetClass;
    ...
    if (_count != 0 && {_weapon == _item || _weaponNonPreset == _item}) then {
		...
		_newContainer addWeaponCargoGlobal [_weaponNonPreset, 1];

        if (_keepAttachments) then {
            _newContainer addItemCargoGlobal [_muzzle, 1];
            _newContainer addItemCargoGlobal [_pointer, 1];
            _newContainer addItemCargoGlobal [_optic, 1];
            _newContainer addItemCargoGlobal [_bipod, 1];

            _magazine params [["_magazineClass", ""], ["_magazineAmmoCount", 0]];
            if (_magazineClass != "") then {
                _newContainer addMagazineAmmoCargo [_magazineClass, 1, _magazineAmmoCount];
            };

            _magazineGL params [["_magazineGLClass", ""], ["_magazineGLAmmoCount", 0]];
            if (_magazineGLClass != "") then {
                _newContainer addMagazineAmmoCargo [_magazineGLClass, 1, _magazineGLAmmoCount];
            };
        };
    } else {
        ...
        if (_weaponNonPreset == _weapon) then {
            _presetAttachments = _weapon call BP_fnc_weaponComponents;
        };
