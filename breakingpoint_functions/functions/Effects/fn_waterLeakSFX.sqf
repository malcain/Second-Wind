/*
	BreakingPoint: SecondWind
	
	water leak sfx. By Malcain & Alias.
*/
...
while {!isNull _leak_obj} do  
{
 ...
 _splash_r setDropInterval 0.1;
	...
	while {_t =_t + 1; _t < 10} do {
		_splash_r say3D ["waterleak_0", 30];
		sleep 5.7+(random 1.4);
	};
