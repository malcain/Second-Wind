...
if (_playerFactionName == "Survivalist") then 
{
	...
	call {
		//Level 4
		if (_playerFactionLevel == 4) exitWith { if (_playerKill) then { _playerPointLoss = 504; } else { _playerPointLoss = 252; }; };
		//Level 3
		if (_playerFactionLevel == 3) exitWith { if (_playerKill) then { _playerPointLoss = 362; } else { _playerPointLoss = 181; }; };
		//Level 2
		if (_playerFactionLevel == 2) exitWith { if (_playerKill) then { _playerPointLoss = 216; } else { _playerPointLoss = 108; }; };
		//Level 1
		if (_survivalist < 72) exitwith {_playerPointLoss = 0};
	...
	if (_playerFactionName == "Hunter" and _playerKill) then
	{
		...
		call {
			...
			//Level 1
			if (_hunter < 25) then {_playerPointLoss = 0} else {_playerPointLoss = 25 };
		};
		_playerPointLoss = _playerPointLoss * BP_Factions_PointsRatio;
