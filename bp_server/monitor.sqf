...
if (worldName == "Altis") then {
	[] spawn BPServer_fnc_realWeatherAltis;
} else {
	[] spawn BPServer_fnc_realWeather;
};
...
call compile preprocessFileLineNumbers format ["\bp_server\maps\%1.sqf",worldName];

//Load Strongholds
call BPServer_fnc_captureLoad;
...
//Lock The Houses
{
	...
	if (_header == "HOUSE") then
	{
		...
		if (!isNull _building && {_buildingName == (typeOf _building)}) then
		{
			...
			for "_i" from 0 to _buildingDoors do 
			{ 
				_lockVar = format ["bis_disabled_Door_%1", _i]; 
				_building setVariable [_lockVar, 1, true]; 
			};
...
//Spawn The Traps
{
	...
	if (_header == "TRAP") then
	{
		...
		//_trap = createMine [_classname, _position, [], 0];
...
_numVehicles =
{
	...
	if (_uniqueID != "0") then
	{
		...
		if (_object isKindOf "Bicycle_F") then {
			{
			...
			if (damage _object >= 0.75) then {
				[_object,"serverSpawn"] call BP_fnc_vehicleKilled;
...
//Spawn Anomalies
call BPServer_fnc_smugglerInitSpawn;
call BPServer_fnc_dynamicLoot;
...
if !(BP_FastBoot) then
{
	_dynamicVeh = [_numVehicles] execVM "\bp_server\threads\BP_dynamicVeh.sqf";
	[_dynamicVeh] call BP_fnc_addThreadHandle;
};

//Server Mgr Thread
_serverMgr = [] execVM "\bp_server\threads\BP_serverMgr.sqf";
