...
#define CLEANUP_PLAYER_TIME 2700
#define CLEANUP_BODY_TIME 120
#define CLEANUP_WPNHOLDER_TIME 300
#define CLEANUP_SIMWPNHOLDER_TIME 1020
#define CLEANUP_LOOT_TIME 1020 //17 min
...
_lastSpawnLoot = diag_tickTime;
_helicrashCount = 2;

//Spawn mission object loot
{
	if ((getPosASLW _x select 2) < -1.5) then {_underwater = true};
	_type = typeOf _x;
	_config = configFile >> "CfgBuildingLoot" >> _type;
	_canLoot = isClass (_config);
	if (_canLoot && {damage _x < 0.85}) then {
		//Loot
		[_x] call BP_fnc_buildingSpawnLoot;
	};
} count allMissionObjects "building";
...
waitUntil
{
	...
	if (BP_ServerSimulationManager) then
	{
		if ((diag_tickTime - _lastSimulationCheck) > SIM_CHECK_INT) then
		{
			...
			//Ground Weapon Holders
			{
				if (_x call BP_fnc_isInDebug) then { 
					...
					{
						//Dead Body Handling
						if (_x isKindOf "BP_DeadBody" or (_x isKindOf "BP_DeadBodyX")) then
						{
			...
			//Simulation On Vehicles
			{
				call
				{
					...
					//Vehicles
					if (_x isKindOf "Car_F" || {_x isKindOf "Air"} || {_x isKindOf "Boat"}) exitWith
					{
						if (!_keep) then 
						{
							if (_inDebug) then {
		...
		//Spawn mission object loot
		if ((diag_tickTime - _lastSpawnLoot) > CLEANUP_WPNHOLDER_TIME) then
		{
			{
				_lastSpawnLoot = diag_tickTime;
				if ((getPosASLW _x select 2) < -1.5) then {_underwater = true};
	...
	if ((diag_tickTime - _lastTimeoutCheck) > 360) then {
		...
		if (count _allplayers > 0) then {
			//Mines
			{
				_x setOwner (owner (selectRandom _allplayers));
			} count allmines;
		
			//locality for creatures
			/*_sharkTypes = ["GreatWhiteShark2","GreatWhiteShark3"];
			_allSharks = entities [_sharkTypes, [], false, false];
			{
				if (local _x && alive _x) then {
					diag_log text format ["shark locality change %1",_x];
					_player = selectRandom _allplayers;
					[_x] joinSilent (group _player);
					group _player selectLeader _shark;
					[_x] remoteExec ["SHARK_fnc_shark_init", _player]; 
				};
			} ForEach _allSharks;*/
		};
		...
		/*{
			...
			if (alive _x && isNull objectParent _x) then {
				//Sharks
				_sharkPlaces = selectBestPlaces [_playerPos, 600, "waterDepth interpolate [2,3,0,1]", 5, 50];
				_sharkTypes = ["GreatWhiteShark2","GreatWhiteShark3"];
				_place = selectRandom _sharkPlaces;
				if (_place select 1 > 0) then {
					_sharkPos = _place select 0;
					_nearSharks = _sharkPos nearEntities [_sharkTypes, 600];
					if (!(_sharkPos isEqualTo []) && {count _nearSharks < 1}) then {
						_type = selectRandom _sharkTypes;
						//_group = createGroup [east, false];
						_group = group _x;
						_shark = _group createUnit [_type, _sharkPos, [], 0, "FORM"];
						_group selectLeader _shark;
						[(netID _shark)] remoteExec ["SHARK_fnc_shark_init", _x, netId _x]; 
						//[_shark] spawn SHARK_fnc_shark_init;
					};
				};
			};
		} ForEach _allplayers;*/
