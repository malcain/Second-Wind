...
if (isObjectHidden _body) exitWith {};
...
_skeletonArray = ["BP_SkeletonRemains1","BP_SkeletonRemains2","BP_SkeletonRemains3"];
if (_playerFactionName == "Hunter" || {_playerFactionName == "Outlaw"} || _playerFactionName == "Scavenger") then {
_remainsType = selectRandom _skeletonArray;
} else {
...
//underwater corpse looting
if (_underwater) then { _remainsType = selectRandom ["BP_SkeletonRemainsUW1","BP_SkeletonRemainsUW2","BP_SkeletonRemainsUW3"] };
...
if (_playerFactionName == "Scavenger") then {
	...
	if (_freshSpawn) then {
		...
		if (_bodyName == _playername) then { 
			...
			_pointsChange = 50 * (0.6 + 0.1 * (cntStrongholds select 1));
...
//underwater
if (underwater _remains) then {
	//_x addAction ["Tow Up", { _this spawn BP_fnc_ropeAttachObject },_x, 1, true, true, "", "", 3];
	[_remains, ["Tow Up", {_this spawn BP_fnc_ropeAttachObject},[], 1, true, true, "", "", 3]] remoteExec ["addAction", -2, true];
};