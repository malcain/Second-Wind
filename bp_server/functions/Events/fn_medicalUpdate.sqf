...
_isUnconscious = _unit getVariable ["med_unconscious", false];
_perpetrator = _unit getVariable ["hostage_perpetrator", "0"];

if ((_isHostage or _isUnconscious) && _perpetrator != "0" && _event == "medGut") then
{
...
switch (_event) do {
	...
	case "medSurgeryDog": {
		...
		//{
			...
			(owner _medic) publicVariableClient "BP_MedicalEvent";
			//Nomad dog heal points gain
	...
	case "medGut": 
	{
		...
		_isPlayer = (isPlayer _unit or (_unit isKindOf "BP_Player" && !_isZombie));
			...
			if (alive _unit) then
			{
				if (_unitLevel == 0) then {
					...
					if (_medicFaction == "Hunter") then {
						_pointsChange = (_pointsChange * (0.55 + (_unitLevel * 0.15))) max 150;
					};
				};
				...
				_unitPointLoss = 0;
				// Log Death Message
				["playerDied: Class: %1 | Faction: %2 | Level: %3 | Victim: %4",_unitClassID,_unitFaction,_unitLevel,_unit] call BP_fnc_debugConsoleFormat;
				
				// Lose Survivalist Points on Death
				if (_unitFaction == "Survivalist") then 
				{
					_survivalist = _unit getVariable ["survivalist",0];
					call {
						//Level 4
						if (_unitLevel == 4) exitWith { _unitPointLoss = 604; };
						//Level 3
						if (_unitLevel == 3) exitWith {_unitPointLoss = 472; };
						//Level 2
						if (_unitLevel == 2) exitWith {_unitPointLoss = 252; };
						//Level 1
						if (_unitLevel == 1) then {
							if (_survivalist < 112) exitwith {_unitPointLoss = _survivalist};
							 _unitPointLoss = 112;
						};
					};
					//_unitPointLoss = _unitPointLoss * BP_Factions_PointsRatio;
					_unit setVariable ["survivalist",(_survivalist - _unitPointLoss)];
				} else {
					// Lose Hunter Points on Death
					if (_unitFaction == "Hunter") then
					{
						_hunter = _unit getVariable ["hunter",0];
						call {
							//Level 3 and 4
							if (_unitLevel >= 3) exitWith { _unitPointLoss = 100 };
							//Level 2
							if (_unitLevel == 2) exitWith { _unitPointLoss = 50 };
							//Level 1
							if (_hunter < 25) then {_unitPointLoss = _hunter} else {_unitPointLoss = 25 };
						};
						//_unitPointLoss = _unitPointLoss * BP_Factions_PointsRatio;
						_unit setVariable ["hunter",(_hunter - _unitPointLoss)];
					} else {
						// Lose scavenger Points on Death
						if (_unitFaction == "Scavenger") then
						{
							_scavenger = _unit getVariable ["scavenger",0];
							call {
								//Level 3 and 4
								if (_unitLevel >= 3) exitWith { _unitPointLoss = 70 };
								//Level 2
								if (_unitLevel == 2) exitWith { _unitPointLoss = 50 };
								//Level 1
								if (_unitLevel == 1) then {
									if (_scavenger < 30) then {_unitPointLoss = _scavenger} else {_unitPointLoss = 30 };
								};
							};
							//_unitPointLoss = _unitPointLoss * BP_Factions_PointsRatio;
							_unit setVariable ["scavenger",(_scavenger - _unitPointLoss)];
						};
					};
				};
			};
