...
_thunder_steroids	= _this select 5;
_delay_thunder		= _this select 6;

/*
if(_thunder_steroids) then {
	delay_thunder = _delay_thunder;
...
_raport = 360/_direction_monsoon;
_raport = round (_raport * (10 ^ 2)) / (10 ^ 2);
//hint str _raport;
...
if ((_raport <= 2) and (_raport >= 1.33)) then {fctx = -1; fcty = -1};
...
while {incr} do {
	...
	winx = floor ((inx*fctx)/2.5);
	winy = floor ((iny*fcty)/2.5);
	setWind [winx,winy,true];
};

[_debris_branches,_rain_fog] remoteExec ["BP_fnc_monsoonEffect",-2,true];
//[[_debris_branches,_rain_fog],"AL_monsoon\alias_monsoon_effect.sqf"] remoteExec ["execVM",0,true];

/*if(_thunder_steroids) then {
	_rand_pl = [] execVM "AL_monsoon\alias_hunt.sqf";
	...
	[] spawn {
		...
		while {al_monsoon_om} do 
		{
			...
			//[[poz_f_1,_tip_tunet,_numar_sclipiri,_sound_tunet,_storm_clouds],"AL_monsoon\thunder_storm.sqf"] remoteExec ["execVM"];
			[poz_f_1,_tip_tunet,_numar_sclipiri,_sound_tunet,_storm_clouds] call BPServer_fnc_monsoonThunder;
			sleep _delayth2;
		};
	};
};*/
...
if (_effect_on_objects) then {
	...
	while {al_monsoon_om} do {
		_//rand_pl = [] execVM "AL_monsoon\alias_hunt.sqf";
		//waitUntil {scriptDone _rand_pl};
		call BPServer_fnc_monsoonHunt;
	...
	// interval object blow
		...
		//_ar_obj_eligibl = [];
		waitUntil {sleep 10; !isNil "hunt_alias"};
		_ar_obj_eligibl = nearestObjects [hunt_alias,["LandVehicle","Man","Air","Wreck"],50];
		
		/*{if((_x isKindOf "LandVehicle") or (_x isKindOf "Man") or (_x isKindOf "Air") or (_x isKindOf "Wreck")) then 
			{_ar_obj_eligibl pushBack _x;};
		} foreach al_nearobjects;*/
		
		sleep 1;
		
		// alege obiect
		_blowobj= _ar_obj_eligibl call BIS_fnc_selectRandom;
		...
		[] spawn {
			...
			//["AL_monsoon\camera_shake.sqf"] remoteExec ["execVM"];
			[_debris_branches,_rain_fog] remoteExec ["BP_fnc_monsoonCamShake",-2];
		};
		
		if !(_blowobj isEqualTo []) then {
			_xblow	= 0.1+random 5;
			_yblow	= 0.1+random 5;
