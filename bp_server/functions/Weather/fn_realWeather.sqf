	...
	*/
	...
	_hour1 = 3;
	_hour2 = 4 + round (random 2);
	_minute = random 40;
	if (!_timeState) then {
	_startingdate = [2024, 06, 21, _hour1, _minute];
	} else {
	_startingdate = [2024, 06, 21, _hour2, _minute];
	}; 
				...
				if (_rain >= 0.1) then {
					if (_overcast >= 0.8) then {
						...
						if (_overcast >= 0.67) then {
							...
							_fog = [(_rain*0.4) , 0.02, _fogHeight];
						};
					};
				} else { //Fog:
					if(_sunOrMoon < 1) then {
					//_fog = [0.9, 0.17, random 10]; //Evening valley fog
					_fog = [0.7, 0.07 + random 0.02, _fogHeight + random 10];
					} else {
