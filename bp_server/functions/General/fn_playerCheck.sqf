...
//Survivalist Points change over time
...
if (_class == 5) then {
	...
	if ((diag_tickTime - _lastPointCheck) > 90) then 
	{
		if ((_player distance _lastPos) > 1) then
		{
			...
			/*} else {
				//Ranger & Outlaw lack of Stronghold point loss
				_playerFactionLevel = _player call BP_fnc_getFactionLevel;
				if (_class == 1 && {cntStrongholds select 0 < 1} && {_playerFactionLevel > 1}) then {
					_pointsChange = -1;
					[_player,_pointsChange,false] call BPServer_fnc_addFactionPoints;
				} else {
					if (_class == 2 && {cntStrongholds select 1 < 1} && {_playerFactionLevel > 1}) then {
						_pointsChange = -1;
						[_player,_pointsChange,false] call BPServer_fnc_addFactionPoints;
					};
				};
