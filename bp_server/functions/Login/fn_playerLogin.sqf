...
#define POINTMAX 12000

scriptName "BP_playerLogin";

// Load Thread Params
params ["_playerID","_player","_playerName","_clientID","_classCheck","_timerAuthed"];
...
_primary params ["","_medical","_stats","_currentState","_worldspace","_class","_ranger","_outlaw","_hunter","_nomad","_survivalist","_scavenger","_undead","_clan"];

if (_classCheck > 0) then {_class = _classCheck};

_factionName = _class call BP_fnc_getFactionName;
["Class: %1 Faction: %2",_class,_factionName] call BP_fnc_debugConsoleFormat;

//Create appropriate unit. When there is no class to process (respawn), default unit is created and then replaced in BP_fnc_checkFaction.
_uniformClass = "C_man_polo_BP";

if (_class <= 0) exitwith {
//Send Login Data
[_clientID,[["_class","_ranger","_outlaw","_hunter","_nomad","_survivalist","_scavenger","_undead"]]] call BPServer_fnc_sendLogin;
};
...
//Create Unit
_group = createGroup civilian;
_body = _group createUnit [_uniformClass, (markerPos BP_DebugMarker), [], 0, "CAN_COLLIDE"];
_body triggerDynamicSimulation true;
_body allowDamage false;
_body setVariable ["bodyName",_playerName,true];
_body setVariable ["CharacterID",_charID];
...
if (_scavenger < POINTMIN or _scavenger > POINTMAX) then { _scavenger = 0; };
	...
	if(true) then {
		...
		if !(_squadData isEqualTo []) then
		{
			...
			if !(_squadEmail isEqualTo "") then
			{
				if (_class == 0) then
				{
					_legionDataVarName = format["BP_LDXML_Bandit:%1:%2", _squadName, _squadEmail];//None
				}
				else
				{
					...
					else
					{
						...
						{
							_legionDataVarName = format["BP_LDXML_Outlaw:%1:%2", _squadName, _squadEmail];//Outlaw-Scavenger
						};
					};
				};
...
//Send Login Data
[_clientID,[_playerID,netID _body,_charID,_isNew,_randomSpot,_medical,_survival,[_class,_ranger,_outlaw,_hunter,_nomad,_survivalist,_scavenger,_undead],_dogData]] call BPServer_fnc_sendLogin;
