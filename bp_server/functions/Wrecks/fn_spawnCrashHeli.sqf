...
private ["_position","_preWaypointPos","_wp","_index","_itemType","_lootPos","_nearby","_RandStartPos","_CentreMarker","_CentreRadius","_CentreMarkerPos","_CentreVehicleID","_CentreVehicle","_preWaypoints","_guaranteedLoot","_randomizedLoot","_rndRadius","_crashDamage","_heliModel","_crashModel","_lootRadius","_heliStart","_crashName","_lootTable","_roll", "_position","_PosList","_PosSelect","_crashPos","_startTime","_crashwreck","_landingzone","_aigroup","_helipilot","_wp2","_wp3","_pos","_crash","_num","_config","_itemTypes","_weights","_cntWeights","_endTime"];
...
_PosList = selectBestPlaces [_position,_rndRadius,"(meadow) * (1 - houses) * (1 - waterDepth)",10,25];
if (_PosList isEqualTo []) exitWith {};
_PosSelect = selectRandom _PosList;
_crashPos = _PosSelect select 0 findEmptyPosition [0,25];
if (_crashPos isEqualTo []) exitWith {};


//Create an Invisible Landingpad at the Crashside-Coordinates
_landingzone = createVehicle ["Land_HelipadEmpty_F", [_crashPos select 0, _crashPos select 1,0], [], 0, "CAN_COLLIDE"];
...
waituntil {(_crashwreck distance _crashPos) <= 150 || not alive _crashwreck || (getPosATL _crashwreck select 2) < 5 || (damage _crashwreck) >= _crashDamage};
...
for "_x" from 1 to _num do
{
	...
	_lootPos = [_pos, ((random 4) + (sizeOf(_crashModel) * _lootRadius)), random 360] call BIS_fnc_relPos;
...
["spawnCrashSite: Crash completed! Wreck at: %2 - Runtime: %1 Seconds || Distance from calculated POC: %3 meters~0001", round(_endTime), str(getPos _crash), round(_crashPos distance _crash)] call BP_fnc_debugConsoleFormat;
