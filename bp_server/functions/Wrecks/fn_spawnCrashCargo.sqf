		...
		if (!surfaceIsWater _preWaypointPos) then
		{
			...
			if (!isNull _cargo) then
			{
				...
				_handle = [_para,_cargo] spawn {
					...
					waitUntil {sleep 1; ((getPosATL _cargo) select 2 < 0.5 || (isTouchingGround _cargo) || ((diag_tickTime - _fallTime) > 30))};
...
waituntil {sleep 2; (_crashwreck distance _position) <= 150 || not alive _crashwreck || (getPosATL _crashwreck select 2) < 5 || (damage _crashwreck) >= _crashDamage};
...
waitUntil {sleep 2; isTouchingGround _crashwreck || ((diag_tickTime - _fallTime) > 30)};
