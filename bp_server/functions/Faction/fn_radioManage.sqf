...
switch (_channel) do
{
	...
	case 4: { if (_bool) then {BP_Radio_Survivalist radioChannelAdd [_unit];} else {BP_Radio_Survivalist radioChannelRemove [_unit];};};
	case 5: { if (_bool) then {BP_Radio_Scavenger radioChannelAdd [_unit];} else {BP_Radio_Scavenger radioChannelRemove [_unit];};};
};