// JBOY_berserkerMelee.sqf
// _dmy = [jojo, repairDude] execVM "JBOY\JBOY_berserkerMelee.sqf";
...
_n = [_slasher, _victim] spawn {
    ...
    while {alive _slasher and (animationState _slasher find "unconscious") == -1 and alive _victim} do  
    {
    /*
        ...
        {
        */
            if (_slasher distance _victim > 2.5) then
            {
                _slasher stop false;
                ...
                _future = time + 20;
//systemchat "wait after moveTo";
               waituntil {sleep .5; _slasher distance _pos <  2.5 or !alive _slasher or !alive _victim or (animationState _victim find "unconscious") > -1 or time >= _future};
            } else 
            {
...
//player sidechat "distance < 2";
        ...
        {
            if (_slasher distance _victim <= 4) then
            {
                _slasherSpeaker setpos getpos _slasher;
                _slasherSpeaker say3D[selectRandom ["slash1","slash2","slash3","slash4"],15,1];
                _slasher setdir ([_slasher,_victim] call BIS_fnc_dirTo);
