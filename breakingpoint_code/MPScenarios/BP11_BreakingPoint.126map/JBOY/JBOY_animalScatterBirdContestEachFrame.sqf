...
while {alive _critter} do
{
    ...
    //hint format ["Scatter, type of %1",typeOf _critter];
            ...
            {
                JBOY_TimeLimit = time + 4;
                JBOY_Critter = _critter;
                dostop cojon;
                ...
                _iterations = 0;
                JBOY_relPos = cojon worldToModel position _critter;
                JBOY_TARGETED = false;
                _falling = false;
/*                while {not JBOY_TARGETED} do
                (    (
                       ...
                       and time > JBOY_TimeLimit
                     )
                       //or time < JBOY_TimeLimit
                        )
                        //and time > JBOY_TimeLimit  // allow time for bird to higher
                (((abs(getdir cojon - ([cojon, _critter modelToWorld [0,3,0]] call BIS_fnc_dirTo))) > 4 
                        and getposasl _critter select 2 > (getposasl cojon select 2) + 2)
                        or getposatl _critter select 2 > 1.2
                       //or time < JBOY_TimeLimit
                        ...
                        //and time > JBOY_TimeLimit  // allow time for bird to higher
                        and time < JBOY_TimeLimit +7  // prevent from waiting forever when bird stuck in tree
                      } do */
                 _height = 0;
                 JBOY_TimeStart = time;
                 doStop cojon;
...
cojon addEventHandler ["animDone", 
{
	pos1 = pos2; 
...
onEachFrame
{
//systemchat format ["in onEachFrame"];
    CojonSpinning = true;
    cojon disableAI "ANIM";
    _height = 0;
    _dir = 0;
    if (!CojonSpinning) then 
	{
		onEachFrame {}; 
		cojon removeAllEventHandlers "animDone";
	} else
    {
systemchat format ["JBOY_TARGETED=%1, TIME=%2, JBOY_TimeLimit=%3, JBOY_Critter=%4,JBOY_relPos=%5", JBOY_TARGETED, str time, str JBOY_TimeLimit, JBOY_Critter, JBOY_relPos];
                //while {not JBOY_TARGETED and time < JBOY_TimeLimit } do
                 IF (not JBOY_TARGETED 
                    //and time < JBOY_TimeLimit 
                    ) THEN
                {
                    //hint format ["Spin loop cojon dir=%1, %2",getdir cojon, getdir JBOY_Critter];
                    CojonSpinning = true;
                    // if (getposasl JBOY_Critter select 2 < _prevHeight) then 
                    // {
                        // _falling = true;
                    // };
                    // _prevHeight = getposasl JBOY_Critter select 2;
                    _height = getposatl JBOY_Critter select 2;
                    //if  ((cojon weaponDirection (currentWeapon cojon) vectorCos ((position JBOY_Critter) vectorDiff (eyepos cojon))) > 0.999 min (0.975 + (0.00024 *(cojon distance JBOY_Critter)))
                    if  ((abs(getdir cojon - ([cojon, JBOY_Critter modelToWorld [0,3,0]] call BIS_fnc_dirTo))) < .2
                       and _height > .6 and _height < 2.8
                       //and _height > .8 and _height < 2.6
                       //and time > JBOY_TimeStart + (.1 + ((random 3)/10))
                       
                        //and getposasl JBOY_Critter select 2 > (getposasl cojon select 2) + 1
                         //and _falling
                         ) then
                    {
                    ...
                    //if  ((abs(getdir cojon - ([cojon, JBOY_Critter modelToWorld [0,3,0]] call BIS_fnc_dirTo))) < 2
                    if  ((abs(getdir cojon - ([cojon, JBOY_Critter modelToWorld [0,20,0]] call BIS_fnc_dirTo))) < .03
                       and _height < 2.6
                       //and time > JBOY_TimeStart + .5
                   ...
                   //systemchat format ["cone=%1,_height=%2",(cojon weaponDirection (currentWeapon cojon) vectorCos ((position JBOY_Critter) vectorDiff (eyepos cojon))),_height];
                     _iterations = _iterations + 1;
                    
                //hint format ["_relpos=%1",_relPos];
                    //if ((abs(getdir cojon - ([cojon, JBOY_Critter modelToWorld [0,.2,0]] call BIS_fnc_dirTo))) > .1) then
                    if ((   (getdir cojon - ([cojon, JBOY_Critter modelToWorld [0,.2,0]] call BIS_fnc_dirTo))) > .1) then
                    {
                        if (JBOY_relPos select 0 > 0) then
                        {
                            _dir = (getdir cojon) + .1;
                        } else
                        {
    ...
    //systemchat format ["speed _veh=%1",str (speed _veh)];
                ...
                //waituntil {time > JBOY_TimeLimit and getpos _critter select 2 < 2}; // delay cojon shooting to increase liklihood of hitting target
                ...
                //sleep .15;
                if (JBOY_TARGETED and alive _critter and getposatl _critter select 2 > .02) then {
                    cojon forceWeaponFire [ weaponState cojon select 1, weaponState cojon select 2];
