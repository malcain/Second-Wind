...
while {alive _critter} do
{
    ...
    //hint format ["Scatter, type of %1",typeOf _critter];
            ...
            if (random 100 >= 95) then   // random chance to fly directly at shooter
            {
                //player sidechat "fly at shooter";
                
                _dir = [_critter, selectrandom [cojon,valdez]] call BIS_fnc_dirTo;
                _critter setdir _dir;
            };
...
/*                while {not _targeted} do
                ...
                while {not _targeted and time < _timeLimit } do
                {
                         ...
                         ) then
                    {
                        ...
                        sleep .1;
                        if (JBOY_CONTEST_CHEAT == 1 and abs((getposasl _critter select 2) - (eyepos cojon select 2)) < .3) then 
                        {
                            JBOY_CONTEST_CHEAT = JBOY_CONTEST_CHEAT + 1;
                            _critter setdamage 1;
                            cojon setVariable ["vBirdScore", (cojon getVariable "vBirdScore") + 1 ,true];
                            //player sidechat "Cheat!!!! Cheat!!!! Cheat!!!! Cheat!!!! Cheat!!!! Cheat!!!! Cheat!!!! Cheat!!!! Cheat!!!! Cheat!!!! Cheat!!!! ";
                        };
                        cojon forceWeaponFire [ weaponState cojon select 1, weaponState cojon select 2];
                    };
